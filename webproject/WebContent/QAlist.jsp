		...
		<div class="right_side">
			<div class="board-div">
				<table class="board">
					...
					<tr id="tr_final">
						<td colspan="5"><a href="qa_write_view.do">글작성</a></td>
					</tr>
				</table>
				</div>
			<div class="paging">
				<ul class="pagination">
					...
					%>
						...
						<li><a href="QAlist.do?PN=<%= firstPage %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &lt;&lt;  </a> </li>
						
					<%
						if(Integer.parseInt(PN) != 1) {
					%>
					
						<li><a href="QAlist.do?PN=<%= Integer.parseInt(PN) - 1 %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &lt;  </a> </li>
						
					<%
						} else {
					%>
					
						<li><a href="QAlist.do?PN=<%= Integer.parseInt(PN) %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &lt;  </a> </li>
					...
					%>
						<li><a href="QAlist.do?PN=<%= i %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"><%= i %> </a> </li>
					<%
						}
					%>
						<li class="active"><a href="QAlist.do?PN=<%= PN %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"><%= PN %> </a> </li>
					<%
					...
					%>
					<li><a href="QAlist.do?PN=<%= startPage+10 %>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &gt;&gt; </a> </li>
					<%
					} else {
					%>
					<li><a href="QAlist.do?PN=<%=PN%>&search=<%=search%>&searchType=<%=searchType%>" class="bt"> &gt;&gt; </a> </li>
					<%
					}
					%>
				</ul>
